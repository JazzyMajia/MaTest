...
from app.services.exporter import SUPPORTED_FORMATS, build_filename, format_result
...
async def export_sql_query(
    ...
    The query is re-executed (results are not persisted), then formatted
    by the export service and returned as an attachment.
    ...
    Returns:
        File attachment with the formatted query result
    """
    # Get connection
    ...
    # Execute query (results are not persisted, so re-run to export)
    try:
        result = await execute_query_with_service(
            session,
            name,
    ...
    # Format result and return as attachment
    filename = build_filename(name, format)
    return Response(
        content=format_result(result, format),
        media_type=SUPPORTED_FORMATS[format],
