...
import { ExportFormat, exportResultLocally } from "../utils/export";
...
export const Home: React.FC = () => {
  ...
  const doExport = (format: ExportFormat, result: QueryResult) => {
    const filename = exportResultLocally(result, format, selectedDatabase || "query");
    message.success(`Exported ${result.rowCount} rows to ${format.toUpperCase()} (${filename})`);
  };
  ...
  const handleExport = (format: ExportFormat) => {
    ...
    if (queryResult.rows.length > 10000) {
      Modal.confirm({
        ...
        onOk: () => doExport(format, queryResult),
      });
    } else {
      doExport(format, queryResult);
    }
  };

  /** After a successful query, proactively ask whether to export the result. */
  const offerExport = (result: QueryResult) => {
    if (result.rows.length === 0) return;
    notification.open({
      key: "export-offer",
      message: "Export this result?",
      description: `The query returned ${result.rowCount} rows. Would you like to export it as a CSV or JSON file?`,
      duration: 8,
      btn: (
        <Space>
          <Button
            size="small"
            onClick={() => {
              notification.destroy("export-offer");
              doExport("csv", result);
            }}
          >
            EXPORT CSV
